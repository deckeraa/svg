...
public class FHAESAction extends AbstractAction {
	 ...
	 * 
	 * @param name
	 *            - Full human readable name for this action
	 * @param iconName
	 *            - String for the icon file name
	 */
	 ...
	 *            - String for the icon file name
	 * @param shortName
	 *            - Short name of <=13 characters for OSX toolbar buttons
	 */
	...
	/**
	 * Get the short name for this action if specified. A short name is 12 or less characters and typically used for OSX toolbar
	 * buttons
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param button
	 *            the button to associate with
	 * @param defaultValue
	 *            the default boolean value, or null if we should try to automatically figure this out
	 */
	public void connectToggleableButton(AbstractButton button,
		...
		buttonAdapters.add(new ButtonSelectionActionAdapter(button,
				defaultValue));
	}
	...
	private class ButtonSelectionActionAdapter implements
			PropertyChangeListener, ItemListener {
		private AbstractButton button;
		private Boolean lastValue;

		public ButtonSelectionActionAdapter(AbstractButton button,
				Boolean defaultValue) {
			this.button = button;
			...
			} else if ((defaultValue = (Boolean) FHAESAction.this
					.getValue(SELECTED_KEY)) == null)
