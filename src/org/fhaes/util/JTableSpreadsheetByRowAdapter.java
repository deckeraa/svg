...
public class JTableSpreadsheetByRowAdapter implements ActionListener {
	private final static Logger log = LoggerFactory
			.getLogger(JTableSpreadsheetByRowAdapter.class);
	...
	public JTableSpreadsheetByRowAdapter(JTable myJTable) {
		...
		// to copy on some other Key combination.
		mainTable.registerKeyboardAction(this, "Copy", copy,
				JComponent.WHEN_FOCUSED);
		mainTable.registerKeyboardAction(this, "Paste", paste,
				JComponent.WHEN_FOCUSED);
	...
	public void doCopy() {
		...
		/*
		 * log.debug("selected rows: "); for(int i : rowsselected) { log.debug("    - "+i); } log.debug("selected cols: ");
		 * for(int i : colsselected) { log.debug("    - "+i); }
		 * 
	 ...
	 * performed. Paste is done by aligning the upper left corner of the selection with the 1st element in the current selection
	 * of the JTable.
	 */
