 ...
 * <h2>Left to do</h2>
 * <ul>
 * <li>splitBy() would be shorter/simpler if it used a tokenizer (see StringTokenizer, countTokens(), nextToken())
 * 
 * <li>splitBy() might be exactly the same as browser/SearchField.parseIntoWords() -- check it out
 * </ul>
 * 
 * @author Ken Harris &lt;kbh7 <i style="color: gray">at</i> cornell <i style="color: gray">dot</i> edu&gt;
 * @version $Id: StringUtils.java 2976 2012-12-16 21:45:32Z sbr00pwb $
 */
public class StringUtils {
	...
	/**
	 * Pad some text on the left (i.e., right-align it) until it's a specified width. If the text is already longer than the
	...
	/**
	 * Returns a user friendly string of the filename for a file shortened to the number of characters specified. Will try to keep
	 * the first folder and truncate in the middle if possible.
	 * 
	 * @param f
	 * @param charactersallowed
	 * @return
	 */
	public static String shortedFileName(File f, Integer charactersallowed) {
		String path = "";
		String name = "";
		try {
			path = FileUtils.getPath(f.getAbsolutePath());
			name = f.getName();
		} catch (Exception e) {
			return null;
		}
		...
		try {
			if (path.length() + name.length() > charactersallowed) {
				String startpath = path.substring(0,
						path.indexOf(File.separator, 2) + 1);
				String endpath = path
						.substring(path.indexOf(File.separator, 2) + 1);

				Integer charstotrim = path.length()
						- (charactersallowed - name.length() - 4);

				path = startpath + "..." + endpath.substring(charstotrim);
			}
		} catch (IndexOutOfBoundsException e) {
			return (path + File.separator + name).substring(0,
					charactersallowed);
		}
	...
	/**
	 * Given a string, escape any &lt; &gt; &amp; ' " characters for XML. Also, if any characters are unprintable, they're escaped
	 * as raw values (&amp;#xxxx;), so loading the output in any old text editor shouldn't mangle anything.
	 * 
	 * @param input
	 *            a string
	 ...
	 * 
	 * <p>
	 * This is like Java 1.4's String.replaceFirst() method; when I decide to drop support for Java 1.3, I can use that method
	 * instead.
	 * </p>
	 * 
	 * @param str
	 ...
	 * @return the original string, str, with its first instance of source replaced by target
	 */
