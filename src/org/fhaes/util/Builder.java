...
/**
 * Class that provides easy access to images, icons, version and build info.
 * 
 * @author pbrewer
 * 
 */
public class Builder {
	...
	private final static ClassLoader cl = org.fhaes.util.Builder.class
			.getClassLoader();
	private static final Logger log = LoggerFactory.getLogger(Builder.class);

	private static final String VERSION = Builder.class.getPackage()
	...
	public static ImageIcon getImageIcon(String name) {
		...
		if (url != null) {
			...
			log.error("Unabled to find icon " + name
					+ ".  Replacing with the 'missing icon' icon.");
			return null;
		}
				...
				try {
					...
					if (is != null) {
						...
						if (revision != null && revision.length() > 0) {
							log.error("Raw value for 'Implementation-Build' : "
									+ revision);
			...
			while (resEnum.hasMoreElements()) {
				try {
					...
					if (is != null) {
						Manifest manifest = new Manifest(is);
						Attributes mainAttribs = manifest.getMainAttributes();
						revision = mainAttribs
								.getValue("Implementation-Build-Timestamp");
						if (revision != null && revision.length() > 0) {
							return revision;
						}
					}
