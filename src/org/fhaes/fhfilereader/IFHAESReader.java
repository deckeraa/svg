...
/**
 * This interface describes the methods that need to be implemented by a class offering the ability to read fire history data from
 * data files. This interface should not be implemented directly but instead through extending the
...
public interface IFHAESReader {
	 ...
	 * @return
	 */
	public abstract ArrayList<ArrayList<Integer>> getEventDataArrays(
	 ...
	 * @return
	 */
	public abstract ArrayList<ArrayList<Double>> getFilterArrays(
			EventTypeToProcess eventType);
	...
	/**
	 * Get an array containing the index (not year) of the innermost (earliest) ring for each sample. The array will contain one
	...
	/**
	 * Returns a 2D array containing information on when the samples are in recording status or not. The array is of size - sample
	 * number x years in file. The array contains Integers with the meaning:
	 * <ul>
