...
/**
 * A wrapper around a preference string Defaults to autocommit (you set a preference value, it is saved automatically)
 * 
 * This class is not suitable for generic-ization because compile-time checks are useless; the underlying prefs class doesn't use
 * generics. Thus, OBJTYPE is only used to type-check setValue and getValue (and remove ugly casting)
 * 
 * @author lucasm
 * 
 */
...
public abstract class PrefWrapper<OBJTYPE> {
	...
	public void commit() {
		...
		/*
		 * else if(baseClass == Dimension.class) App.prefs.setDimensionPref(getPrefName(), (Dimension) prefValue); else
		 * if(baseClass == Color.class) App.prefs.setColorPref(getPrefName(), (Color) prefValue);
		 */
			...
			App.prefs.setAnalysisTypePref(getPrefName(),
					(AnalysisType) prefValue);
		} else if (baseClass == FireFilterType.class) {
			App.prefs.setFireFilterTypePref(getPrefName(),
					(FireFilterType) prefValue);
		} else if (baseClass == EventTypeToProcess.class) {
			App.prefs.setEventTypePref(getPrefName(),
					(EventTypeToProcess) prefValue);
		} else if (baseClass == AnalysisLabelType.class) {
			App.prefs.setAnalysisLabelTypePref(getPrefName(),
					(AnalysisLabelType) prefValue);
		} else if (baseClass == ResamplingType.class) {
			App.prefs.setResamplingTypePref(getPrefName(),
					(ResamplingType) prefValue);
		} else if (baseClass == Double.class) {
			App.prefs.setDoublePref(getPrefName(), (Double) prefValue);
		} else if (baseClass == NoDataLabel.class) {
			App.prefs
					.setNoDataLabelPref(getPrefName(), (NoDataLabel) prefValue);
		} else
			throw new IllegalArgumentException(
					"I don't know how to save a pref for type " + baseClass);
	...
	private void load() {
		 ...
		 */
			...
			prefValue = App.prefs.getIntPref(getPrefName(),
					(Integer) defaultValue);
		/*
		 * else if(baseClass == Dimension.class) prefValue = App.prefs.getDimensionPref(getPrefName(), (Dimension) defaultValue);
		 * else if(baseClass == Color.class) prefValue = App.prefs.getColorPref(getPrefName(), (Color) defaultValue);
		 */
		else if (baseClass == Boolean.class)
			prefValue = App.prefs.getBooleanPref(getPrefName(),
					(Boolean) defaultValue);
		/*
		 * else if(baseClass == Double.class) prefValue = App.prefs.getDoublePref(getPrefName(), (Double) defaultValue);
		 */
		else if (baseClass == AnalysisType.class) {
			prefValue = App.prefs.getAnalysisTypePref(getPrefName(),
					(AnalysisType) defaultValue);
		} else if (baseClass == FireFilterType.class) {
			prefValue = App.prefs.getFireFilterTypePref(getPrefName(),
					(FireFilterType) defaultValue);
		} else if (baseClass == EventTypeToProcess.class) {
			prefValue = App.prefs.getEventTypePref(getPrefName(),
					(EventTypeToProcess) defaultValue);
		} else if (baseClass == AnalysisLabelType.class) {
			prefValue = App.prefs.getAnalysisLabelTypePref(getPrefName(),
					(AnalysisLabelType) defaultValue);
		} else if (baseClass == ResamplingType.class) {
			prefValue = App.prefs.getResamplingTypePref(getPrefName(),
					(ResamplingType) defaultValue);
		} else if (baseClass == Double.class) {
			prefValue = App.prefs.getDoublePref(getPrefName(),
					(Double) defaultValue);
		} else if (baseClass == NoDataLabel.class) {
			prefValue = App.prefs.getNoDataLabelPref(getPrefName(),
					(NoDataLabel) defaultValue);
		} else {
			throw new IllegalArgumentException(
					"I don't know how to load a pref for type " + baseClass);
		}
	}
