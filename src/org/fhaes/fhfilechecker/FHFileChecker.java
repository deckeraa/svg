...
public class FHFileChecker extends JFrame {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (fileBrowseReturn == 0) {
						...
						JOptionPane.showMessageDialog(
								null,
								" You have selected "
										+ fileBrowse.getSelectedFiles().length
		...
		if (buttonRun == null) {
			...
			buttonRun.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					doCheck(buttonRun, inputFile, null,
							jcheckformatreport.isSelected(),
							jcheckindivreport.isSelected());
	...
	public boolean doCheck(Component parent, File[] inputFile, File outputFile,
			Boolean jcheckformatreport, Boolean jcheckindivreport) {
		...
		if (run) {
			...
			for (int i = 0; i < inputFile.length; i++) {
				...
				// Checking each file
				/*
				 * The check for the format of the fhx file NOTE: the heading of the file with the metadata is not being check.
				 * Must check with Elaine about the comment lines. 1. the file extension is not check also. 2. First and most
				 * important line is the FHX2 FORMAT or FIRE2 format. 3. next line must contain the following integer data (the
				 * integer format will be check) a. Beginning year: this is the beginning year for the fire data file integer b.
				 * number of sample in the data: number of columns integer c. longest code of all samples: integer 4. black (empty
				 * line) must come before the fire data start. 5. fire data : I will check for the symbols lower/Upper case
				 * letters | { } [ ] . a. I will check that only this symbols are used in the data b. I will check that the data
				 * is the correct length ie same as the number of samples Warning c. all the samples should have a starting point
				 * either [ , { or Uppercase letter (fire) Warning d. all the samples should have a closing point either ] or }
				 * Warning 6. A report on each file will be created.
				 */

				formatReport = formatReport + "\n"
						+ " FORMAT REPORT FOR FILE: "
						+ myReaderArray.get(i).getName() + "\n";
				formatReport = formatReport + "\t"
				...
				if (true) {
					...
					formatReport = formatReport + "\t"
							...
							+ myReaderArray.get(i).getFileFormat() + "\n";
					// get the next line information
					if (myReaderArray.get(i).getFirstYear().toString()
							.matches(numericalDataint)) {
						formatReport = formatReport + "\t"
								+ "The first year of your data is: "
					...
					log.debug("size of badlines: "
							+ myReaderArray.get(i).getBadDataLineNumbers()
									.size());
					log.debug("size of data is "
							+ myReaderArray.get(i).getData().size());
					log.debug("size of rwodata is "
							+ myReaderArray.get(i).getRawRowData().size());
					log.debug("getNumberofseries = "
							+ myReaderArray.get(i).getNumberOfSeries()
									.toString());

					if ((myReaderArray.get(i).getNumberOfSeries().toString()
							.matches(numericalDataint))
							&& (myReaderArray.get(i).passesBasicSyntaxCheck())) {
						formatReport = formatReport + "\t" + "Expect "
								+ myReaderArray.get(i).getNumberOfSeries()
						...
						if (myReaderArray.get(i).getBadDataLineNumbers().size() == 0) {
							...
							for (int j = 0; j < myReaderArray.get(i)
									.getBadDataLineNumbers().size(); j++) {
								log.debug("bad line number "
										+ myReaderArray.get(i)
												.getBadDataLineNumbers().get(j)
												.intValue());
								log.debug("bad line length "
										+ myReaderArray
												...
												.get(myReaderArray
														...
														.get(j).intValue())
												.length());
								log.debug("Number of series: "
										+ myReaderArray.get(i)
												.getNumberOfSeries());
								if ((myReaderArray
										.get(i)
										.getRawRowData()
										.get(myReaderArray.get(i)
												.getBadDataLineNumbers().get(j)
												.intValue()).length()) > myReaderArray
									...
									formatReport = formatReport
											...
											+ myReaderArray.get(i)
													.getBadDataLineNumbers()
													.get(j).intValue()
											+ " on the fire matrix has more samples than "
											+ myReaderArray.get(i)
													.getNumberOfSeries() + "\n";
									passFormat = false;
								} else {
									formatReport = formatReport
											...
											+ myReaderArray.get(i)
													.getBadDataLineNumbers()
													.get(j).intValue()
											+ " on the fire matrix has less samples than "
											+ myReaderArray.get(i)
													.getNumberOfSeries() + "\n";
									passFormat = false;
								}
					...
					if (myReaderArray.get(i).getLengthOfSeriesName().toString()
							.matches(numericalDataint)) {
						formatReport = formatReport + "\t"
								+ "Expect the longest ID code to be "
								+ myReaderArray.get(i).getLengthOfSeriesName()
								+ " characters long." + "\n";
					} else {
						formatReport = formatReport
								+ "\t"
					...
					if (myReaderArray.get(i).passesBasicSyntaxCheck()) {
						...
						for (int j = 0; j < myReaderArray.get(i).getData()
								.size(); j++) {
							if (myReaderArray.get(i).getData().get(j)
									.matches(fhx2dataline)) {
								...
								formatReport = formatReport
										+ "\t"
										+ "there is something wrong with at least one of the line in the data"
										+ "\n";
								passFormat = false;
							}
						}
						for (int j = 0; j < myReaderArray.get(i)
								.getNumberOfSeries(); j++) {
							if ((myReaderArray.get(i)
									.getStartYearIndexPerSample()[j] == -1)) {
						...
						if (linecounter == (myReaderArray.get(i).getLastYear()
								- myReaderArray.get(i).getFirstYear() + 1)) {

							formatReport = formatReport + "\t"
									+ "The beginning year of the data is: "
									+ myReaderArray.get(i).getFirstYear()
									+ " The end year of your data is: "
									+ myReaderArray.get(i).getLastYear()
									+ " for a total of : " + linecounter
						...
						formatReport = formatReport
								+ "\t"
								+ "The mandatory blank line before starting the fire data was not found"
								+ "\n";
						passFormat = false;
					}
				...
				if (passFormat) {
					...
					formatReport = formatReport
							+ "\t"
							+ "SORRY! this file failed at least one format condition Therefore: THE "
							+ myReaderArray.get(i).getName()
							+ " FILE FAILED TO PASS THE FORMAT TEST" + "\n";
				}
				...
				if (jcheckindivreport && passFormat) {
					formatReport = formatReport + "\n\n"
							+ " INDIVIDUAL SAMPLE SUMMARY FOR FHX FILE :"
							+ myReaderArray.get(i).getName() + "\n";
					...
					for (int k = 0; k < myReaderArray.get(i)
							.getNumberOfSeries(); k++) {
						...
						formatReport = formatReport
								...
								+ "\t"
								+ "Code: "
								+ myReaderArray.get(i).getSeriesNameArray()
										.get(k) + "\n";
						if (myReaderArray.get(i).getPithIndexPerSample()[k] != -1) {
							formatReport = formatReport
									...
									+ "Pith Ring: "
									+ (myReaderArray.get(i)
											.getPithIndexPerSample()[k] + myReaderArray
											.get(i).getFirstYear()) + "\n";
						}
						if (myReaderArray.get(i).getInnerMostperTree()[k] != -1) {
							formatReport = formatReport
									...
									+ "Inner Ring: "
									+ (myReaderArray.get(i)
											.getInnerMostperTree()[k] + myReaderArray
											.get(i).getFirstYear()) + "\n";
						}
						if (myReaderArray.get(i).getBarkIndexPerTree()[k] != -1) {
							formatReport = formatReport
									...
									+ "Bark Ring: "
									+ (myReaderArray.get(i)
											.getBarkIndexPerTree()[k] + myReaderArray
											.get(i).getFirstYear()) + "\n";
						}
						if (myReaderArray.get(i).getOutterMostperTree()[k] != -1) {
							formatReport = formatReport
									...
									+ "Outer Ring: "
									+ (myReaderArray.get(i)
											.getOutterMostperTree()[k] + myReaderArray
											.get(i).getFirstYear()) + "\n";
						}

						FyearperSampletemp = new ArrayList<Integer>();
						// FIyearperSampletemp = new ArrayList<Integer>();
						for (int j = 0; j < myReaderArray.get(i).getYearArray()
								.size(); j++) {
							if ((myReaderArray
									.get(i)
									.getEventDataArrays(
									...
									.get(k).get(j) == 1)) {
								FyearperSampletemp.add((j + myReaderArray
										.get(i).getFirstYear()));
							}
						}
						...
						formatReport = formatReport
								...
								+ "Length of sample: "
								+ ((myReaderArray.get(i)
										.getLastYearIndexPerSample()[k] - myReaderArray
										.get(i).getStartYearIndexPerSample()[k]) + 1)
								+ "\n";
						formatReport = formatReport
								+ "\t"
								+ "Number of recorder years in sample: "
								+ (myReaderArray.get(i)
										.getTotalRecorderYearsPerSample()[k])
								+ "\n";
						formatReport = formatReport + "\t"
						...
						for (int jk = 0; jk < FyearperSample.get(k).size() - 1; jk++) {
							FIyearperSampletemp.add(FyearperSample.get(k).get(
									jk + 1)
						...
						for (int j = 0; j < myReaderArray.get(i)
								.getCalosYearperSample2d().get(k).size(); j++) {
							if ((myReaderArray.get(i).getCalosperSample2d()
									.get(k).get(j) >= 'a')
									&& (myReaderArray.get(i)
											.getCalosperSample2d().get(k)
											.get(j) <= 'z')) {
								formatReport = formatReport
										+ "\t   "
										+ (myReaderArray.get(i)
												.getCalosYearperSample2d()
												.get(k).get(j) + myReaderArray
												.get(i).getFirstYear())
										+ "   "
										+ myReaderArray.get(i)
												.getCalosperSample2d().get(k)
								...
								if (myReaderArray.get(i)
										.getCapsYearperSample2d().get(k).get(0)
										.intValue() == myReaderArray.get(i)
										.getCalosYearperSample2d().get(k)
										.get(j).intValue()) {
									formatReport = formatReport
											+ "\t   "
											+ (myReaderArray.get(i)
						...
						formatReport = formatReport
								+ "\t"
								+ "Total number of fire scars: "
								+ myReaderArray.get(i).getCapsperSample2d()
										.get(k).size() + "\n";
						formatReport = formatReport
								+ "\t"
								+ "Total number of all indicators: "
								+ myReaderArray.get(i)
										.getCalosYearperSample2d().get(k)
										.size() + "\n";
						...
						if (myReaderArray.get(i).getCapsperSample2d().get(k)
								.size() != 0) {
							formatReport = formatReport
									+ "\t"
									+ "Average number years per fire: "
									+ onePlace
											.format((((double) myReaderArray
													.get(i)
													.getTotalRecorderYearsPerSample()[k]) / myReaderArray
													.get(i)
													.getCapsperSample2d()
													.get(k).size())) + "\n";
						} else {
						...
						if (FIyearperSample.get(k).size() != 0) {
							formatReport = formatReport
									+ "\t"
									+ "Sample mean fire interval: "
									+ onePlace
											.format((((double) fisumtemp) / FIyearperSample
													.get(k).size())) + "\n";
						} else {
							formatReport = formatReport + "\t"
									+ "Sample mean fire interval: " + " NA \n";
						}

						totalRecorderYearsPerSite = totalRecorderYearsPerSite
								+ myReaderArray.get(i)
										.getTotalRecorderYearsPerSample()[k];
						totalFireScarsPerSite = totalFireScarsPerSite
								+ myReaderArray.get(i).getCapsperSample2d()
										.get(k).size();
						totalAllIndicatorsPerSite = totalAllIndicatorsPerSite
								+ myReaderArray.get(i)
										...
										.size();
						if (FIyearperSample.get(k).size() != 0) {
							totalMeanFireIntervals = totalMeanFireIntervals
									+ (((double) fisumtemp) / FIyearperSample
											.get(k).size());
						}
					...
					/*
					 * find totals for Recorder years per file totals for fire scars per file totals for all indicators avg number
					 * of years per fire: total number of recorder years/total number of fire scars avg number of years per all
					 * injuries: total number of recorder years/total number of all indicators avg all sample mean fire intervals
					 * total number of years with fire percentage of years with fires: total number of years with fire percentage
					 * of years with fire: percentage year MFI
					 */
					for (int j = 0; j < myReaderArray.get(i)
							.getFireEventsArray().size(); j++) {
					...
					formatReport = formatReport + "\n\n"
							...
							+ "\n";
					formatReport = formatReport + "\t"
							+ "Total number of recorder years: "
							+ totalRecorderYearsPerSite + "\n";
					formatReport = formatReport + "\t"
							+ "Total number of fire scars: "
							+ totalFireScarsPerSite + "\n";
					formatReport = formatReport + "\t"
							+ "Total number of all indicators: "
							+ totalAllIndicatorsPerSite + "\n";
					if (totalFireScarsPerSite != 0) {
						formatReport = formatReport
								+ "\t"
								+ "Average number of years per fire: "
								+ onePlace
										.format((((double) totalRecorderYearsPerSite) / totalFireScarsPerSite))
								+ "\n";
					} else {
						formatReport = formatReport + "\t"
								+ "Average number of years per fire: "
					...
					if (totalAllIndicatorsPerSite != 0) {
						formatReport = formatReport
								+ "\t"
								+ "Average number of years per all indicators: "
								+ onePlace
										.format((((double) totalRecorderYearsPerSite) / totalAllIndicatorsPerSite))
								+ "\n";
					} else {
						formatReport = formatReport
								+ "\t"
					...
					formatReport = formatReport
							+ "\t"
							+ "Average all sample mean fire intervals: "
							+ onePlace.format(totalMeanFireIntervals
									/ myReaderArray.get(i).getNumberOfSeries())
							+ "\n";
					formatReport = formatReport + "\t"
							+ "Total number of years with fire: "
							+ totalAllIndicatorsPerSite + "\n";
					formatReport = formatReport
							+ "\t"
							+ "Percentage of years with fire: "
							+ onePlace
									.format(((double) totalYearsWithFirePerSite
											/ linecounter * 100)) + "\n";
					formatReport = formatReport
							+ "\t"
							+ "Percentage of years with without fire: "
							+ onePlace
									.format((100 - ((double) totalYearsWithFirePerSite
											/ linecounter * 100))) + "\n";
					formatReport = formatReport
							+ "\t"
							+ "Percentage of years with MFI: "
							+ onePlace
									.format((100 / ((double) totalYearsWithFirePerSite
											/ linecounter * 100))) + "\n";
			...
			if (outputFile == null) {
				...
				fileBrowse = new JFileChooser(savePath.substring(0,
				 ...
				 */
				fileBrowse
						.setDialogTitle("Select the name and location for the Report (outputfile):");
				/*
				...
				if (fileBrowseReturn == 0) {
					 ...
					 */
					if (l <= 4
							|| !(outputFile.getName().substring(l - 4, l)
									.equals(".txt"))) {
	...
	private JPanel getJPanelBrowse() {
		...
		if (jPanelBrowse == null) {
			...
			jLabelBrowse
					.setText("Click the Browse button to Select fhx file(s): ");
	...
	private JPanel getJPanelReportType() {
		...
		if (jPanelReportType == null) {
			...
			jlabelindivreport
					.setText("Create individual sample summary report for file(s) that passed the format test");
