...
/**
 * Class for calculating various statistical measured based on the Weibull distribution.
 * 
...
public class Weibull {
	...
	private static double[] generateParameters(ArrayList<Double> data) {
		...
		while ((tt > tolerance) && (niter <= maxitr) && (denom > 0.01)) {
			...
			denom = (1 / (m * m))
					+ (((sxm * sxmlnx2) - (sxmlnx * sxmlnx)) / (sxm * sxm));
			mnew = m + (numera / denom);
			//
	...
	public double getExceedencePercentile(Double percentile) {
		...
		return Math.exp(Math.log(-1.0 * Math.log(1.0 - val)) / getShape()
				+ Math.log(getScale()));
	...
	public double[] getExceedenceProbability() {
		...
		double shape = wparam[0];
		double scale = wparam[1];
		double[] fixval = { 0.001, 0.01, 0.025, 0.05, 0.1, 0.125, 0.2, 0.25,
				0.3, 0.333, 0.5, 0.667, 0.7, 0.75, 0.8, 0.875, 0.9, 0.95,
				0.975, 0.99, 0.999 };
		...
		for (int k = 0; k < fixval.length; k++) {
			excprb[k] = Math.exp(Math.log(-1.0 * Math.log(1.0 - fixval[k]))
	...
	public double[] getExceedenceProbability2() {
		...
		for (int k = 0; k < fixval.length; k++) {
			...
			// log.debug("luei"+excprb[k]);
			temp = Math
					.exp((Math.log(-1.0 * Math.log(1.0 - fixval[k])) / shape)
							+ Math.log(scale));
	...
	public double getSigma() {
		...
		xmu = 1 + (1 / shape);
		ymu = 1 + (2 / shape);
		double xsig = test.la_gamma(ymu)
				- (test.la_gamma(xmu) * test.la_gamma(xmu));
	...
	public double getSkew() {
		...
		xmu = 1.0 + (3.0 / shape);
		double numskew = (test.la_gamma(xmu) * Math.pow(scale, 3.0)
				- (3.0 * getMean() * getSigma() * getSigma()) - (getMean()
				* getMean() * getMean()));
	...
	public ArrayList<Double> getWeibullProbability(ArrayList<Double> data) {
		...
		for (int i = 0; i < data.size() - 1; i++) {
			weibullProb.add(Math.exp(-1
					* Math.pow((data.get(i) / scale), shape)));
		}
